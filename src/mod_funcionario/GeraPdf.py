...
import requests
from settings import ENDPOINT_FUNCIONARIO, HEADERS_API
    ...
    def listaTodos(self):
        ...
        try:
            ...
            for row in result[0]:
                ...
                pdf.cell(40, 5, row.get('matricula', ''), 0, 0, 'L')
                pdf.cell(30, 5, row.get('cpf', ''), 0, 0, 'L')
                pdf.cell(30, 5, row.get('telefone', ''), 0, 0, 'L')
                pdf.cell(15, 5, str(row['grupo']), 0, 0, 'L')
                pdf.cell(15, 5, row.get('senha', ''), 0, 1, 'L')

        except Exception as e:
            pdf.multi_cell(190, 5, 'ERRO: ' + str(e), 1, 'J', False)

        pdf.output("C:\\Users\\matheus.felipe\\source\\repos\\Teste\\pastelaria-frontend\\src\\pdfFuncionario.pdf")
